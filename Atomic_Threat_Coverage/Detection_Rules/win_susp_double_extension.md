...
title: Suspicious Double Extension status:  \
description: Detects suspicious use of an .exe extension after a non-executable file extension like .pdf.exe, a set of spaces or underlines to cloak the executable file in spear phishing campaigns \
...
search = (Image="*.doc.exe" OR Image="*.docx.exe" OR Image="*.xls.exe" OR Image="*.xlsx.exe" OR Image="*.ppt.exe" OR Image="*.pptx.exe" OR Image="*.rtf.exe" OR Image="*.pdf.exe" OR Image="*.txt.exe" OR Image="*      .exe" OR Image="*______.exe") | stats values(*) AS * by _time | search NOT [| inputlookup Suspicious_Double_Extension_whitelist.csv] | collect index=threat-hunting marker="sigma_tag=attack.initial_access,sigma_tag=attack.t1193,level=critical"
```
