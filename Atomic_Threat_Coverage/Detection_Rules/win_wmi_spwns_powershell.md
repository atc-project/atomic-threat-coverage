...
```
Get-WinEvent | where {(($_.message -match "ParentImage.*.*\\\\wmiprvse.exe") -and ($_.message -match "Image.*.*\\\\powershell.exe")) } | select TimeCreated,Id,RecordId,ProcessId,MachineName,Message
```
